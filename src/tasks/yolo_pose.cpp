...
namespace yolopose
{
	...
	public:
		...
		virtual void worker(std::promise<bool> &result) override
		{
			...
			if (engine == nullptr)
			{
				LOG_E("yolopose", "Engine %s load failed", file.c_str());
			...
			while (get_jobs_and_wait(fetch_jobs, max_batch_size))
			{
				...
				for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch)
				{
					...
					decode_kernel_invoker(image_based_output, num_bboxes, confidence_threshold_, affine_matrix, output_array_ptr, max_image_bbox, stream_, type_, output_channel_major);
				...
				for (int ibatch = 0; ibatch < infer_batch_size; ++ibatch)
				{
					...
					for (int i = 0; i < count; ++i)
					{
						...
						float *pkeypoint = pbox + 6;
						for (int k = 0; k < NUM_KEYPOINTS; ++k)
						{
							box.keypoints.emplace_back(pkeypoint[0], pkeypoint[1], pkeypoint[2]);
							pkeypoint += 3;
						}
		...
		virtual bool preprocess(Job &job, const cv::Mat &image) override
		{
			...
			{
				LOG_E("yolopose", "tensor_allocator_ is nullptr");
				return false;
			}

			if (image.empty())
			{
				LOG_E("yolopose", "Image is empty");
				return false;
			}
			...
			{
				LOG_E("yolopose", "Tensor allocator query failed.");
				return false;
			}
