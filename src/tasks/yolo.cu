...
namespace yolo
{
	...
	static __global__ void decode_kernel_yolo26(
		...
		int max_objects, cudaStream_t stream, Type type, bool channel_major)
	{
		...
		if (type == Type::v8 || type == Type::v11)
		{
			if (channel_major)
			{
				checkCUDAKernel(decode_kernel_v8_channel_major<<<grid, block, 0, stream>>>(
		...
		else if (type == Type::YOLO26)
		{
			checkCUDAKernel(decode_kernel_yolo26<<<grid, block, 0, stream>>>(
