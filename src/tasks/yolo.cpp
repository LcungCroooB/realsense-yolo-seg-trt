...
namespace yolo
{
    static std::shared_ptr<trt::Tensor> resolve_single_input_tensor(const std::shared_ptr<trt::Infer> &engine)
    {
        if (engine->num_input() == 1)
            return engine->input(0);

        if (engine->is_input_name("images"))
            return engine->tensor("images");

        LOG_E("yolo", "Failed to resolve input tensor. input_count=%d", engine->num_input());
        return nullptr;
    }

    static std::shared_ptr<trt::Tensor> resolve_single_output_tensor(const std::shared_ptr<trt::Infer> &engine)
    {
        if (engine->is_output_name("output"))
            return engine->tensor("output");

        if (engine->num_output() == 1)
        {
            const std::string output_name = engine->get_output_name(0);
            LOG_W("yolo", "Use fallback output tensor: %s", output_name.c_str());
            return engine->output(0);
        }

        LOG_E("yolo", "Failed to resolve output tensor. output_count=%d", engine->num_output());
        return nullptr;
    }
    ...
    static BoxArray cpu_nms(BoxArray &boxes, float threshold)
    {
        ...
        for (int i = 0; i < (int)boxes.size(); ++i)
        {
            ...
            for (int j = i + 1; j < (int)boxes.size(); ++j)
            {
                ...
                auto &b = boxes[j];
                if (b.class_label == a.class_label && iou(a, b) >= threshold)
                    remove_flags[j] = true;
            }
        }
        return output;
    ...
    public:
        ...
        {
            if (type == Type::v5 || type == Type::v8 || type == Type::v11 || type == Type::YOLO26)
            {
                normalize_ = trt::Norm::alpha_beta(1.0f / 255.0f, 0.0f, trt::ChannelType::Invert);
            }
            else if (type == Type::X)
            {
                normalize_ = trt::Norm::None();
            }
            else
            {
                LOG_E("yolo", "Unsupported yolo type: %d", (int)type);
                return false;
            }
        ...
        virtual void worker(std::promise<bool> &result) override
        {
            ...
            if (engine == nullptr)
            {
                LOG_E("yolo", "Engine %s load failed", file.c_str());
            ...
            const int num_box_element = (type_ == Type::YOLO26) ? 6 : 7;

            trt::Tensor affine_matrix_device(trt::DataType::Float32);
            trt::Tensor output_array_device(trt::DataType::Float32);
            int max_batch_size = engine->get_max_batch_size();
            auto input = resolve_single_input_tensor(engine);
            auto output = resolve_single_output_tensor(engine);
            if (input == nullptr || output == nullptr)
            {
                LOG_E("yolo", "Resolve model tensors failed for engine: %s", file.c_str());
                result.set_value(false);
                return;
            }
            ...
            if (type_ == Type::v8 || type_ == Type::v11)
            {
                ...
                LOG_I("yolo", "Detect head layout: %s, dims={%d x %d x %d}",
                      output_channel_major ? "[B,C,N]" : "[B,N,C]",
                      output->size(0), output->size(1), output->size(2));
            }
            else if (type_ == Type::YOLO26)
            {
                num_classes = output->size(2) - 4;
                num_bboxes = output->size(1);
            }
        ...
        virtual bool preprocess(Job &job, const cv::Mat &image) override
        {
            ...
            {
                LOG_E("yolo", "tensor_allocator_ is nullptr");
                return false;
            }

            if (image.empty())
            {
                LOG_E("yolo", "Image is empty");
                return false;
            }
            ...
            {
                LOG_E("yolo", "Tensor allocator query failed");
                return false;
            }
    ...
    void image_to_tensor(const cv::Mat &image, std::shared_ptr<trt::Tensor> &tensor, int ibatch, Type type)
    {
        trt::Norm normalize;
        if (type == Type::v5 || type == Type::v8 || type == Type::v11 || type == Type::YOLO26)
        {
            normalize = trt::Norm::alpha_beta(1.0f / 255.0f, 0.0f, trt::ChannelType::Invert);
        }
        else if (type == Type::X)
        {
            normalize = trt::Norm::None();
        }
        else
        {
            LOG_E("yolo", "Unsupported yolo type: %d", (int)type);
